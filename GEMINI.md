...
- 不得执行会改变 Git 状态的命令，包括 add、commit、push、pull、fetch、merge、rebase、reset、restore、checkout、clean、tag、branch、stash。
- 如果用户要求实现、修复或改代码，Gemini 只能给出审查结论、风险和建议补丁方向，不得直接修改文件。
...
`pwd` 与 Git root 必须都是 `/Users/vita/Vitemis/Packagis`。如果不匹配，停止审查并报告路径问题。
...
Gemini 报告只能写入 `gemini-report/`，文件名必须采用 `MM_DD_YY-HH_MM-xxxx.md`。报告正文必须先写 `MODEL_CHECK_RESULT`，并建议包含 `PATH_CHECK_RESULT`、`FINDINGS`、`FILES_WRITTEN`、`VALIDATION_RESULT`、`UNCERTAINTIES` 和 `NEXT_RECOMMENDED_ACTION`。
